...
namespace rs
{
    ...
    {
        continous_format      , /**< Makes sure that the output frame is exposed as a single continuous buffer */
        native_format           /**< Don't convert buffer to continuous, the user has to handle pitch manually */
    };
    ...
    {
        depth_cap,                      /**< provides depth stream */
        color_cap,                      /**< provides color stream */
        infrared_cap,                   /**< provides infrared stream */
        infrared2_cap,                  /**< provides second infrared stream */
        fish_eye_cap,                   /**< provides wide field of view (fish-eye) stream */
    ...
    typedef std::function<void(motion_data)> motion_data_function;

    class motion_callback : public rs_motion_callback
    {
      motion_data_function on_event_function;
    public:
      explicit motion_callback(motion_data_function on_event) : on_event_function(on_event) {}

      void on_event(rs::rs_motion_data e) override
        {
    ...
    public:
      ...
      void on_event(rs::rs_timestamp_data data) override
        {
    ...
    public:
      explicit frame_callback(std::function<void(rs::frame)> on_frame) : on_frame_function(on_frame) {}
        ...
        {
	  on_frame_function(std::move(rs::frame(device, fref)));
        }
    ...
    public:
      ...
      void set_frame_callback(rs::stream stream, std::function<void(rs::frame)> frame_handler)
        ...
        /// \return                       the framerate of the stream, in frames per second
      void enable_motion_tracking(motion_data_function motion_handler,
    ...
    enum class log_severity : int
    {
        ...
        none_log  = 5, /**< No logging will occur */
    };

  typedef std::function<void(log_severity, const char * message> log_function;
